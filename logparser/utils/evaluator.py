...
def evaluate(groundtruth, parsedresult, filename=""):
    ...
    # get_wrong_word(df_groundtruth, df_parsedlog, filename)
    (precision, recall, f_measure, accuracy) = get_accuracy(df_groundtruth['EventId'], df_parsedlog['EventId'], filename)

    print('Precision: %.4f, Recall: %.4f, F1_measure: %.4f, Parsing_Accuracy: %.4f'%(precision, recall, f_measure, accuracy))
    return f_measure, accuracy


def get_wrong_word(df_groundtruth, df_parsedlog, filename):
    groundtruth = df_groundtruth[["EventId", "EventTemplate"]]
    parsedlog = df_parsedlog[["EventId", "Content", "LineId"]]
    with open(os.path.join(filename + '_result.txt'), "a+", encoding="utf8") as f:
        f.write(filename + " \n")
    for _, i in groundtruth.iterrows():
        for _, j in parsedlog.iterrows():
            with open(os.path.join(filename + '_result.txt')) as f:
                f.write(i["LineId"] + "  content: " + i["Content"] + "   template: " + i["EventTemplate"] + "\n")


def get_accuracy(series_groundtruth, series_parsedlog, filename="", debug=False):
    ...
    # with open("0.txt", "a+", encoding="utf8") as f:
    #     f.write(filename+"\n")
    ...
    for parsed_eventId in series_parsedlog_valuecounts.index:
        ...
        error = True
        data = [i for i in data if i not in logIds]
        if series_groundtruth_logId_valuecounts.size == 1:
            groundtruth_eventId = series_groundtruth_logId_valuecounts.index[0]
            x = series_groundtruth[series_groundtruth == groundtruth_eventId]
