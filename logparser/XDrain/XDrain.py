"""
Description : This file implements the XDrain algorithm for log parsing
Author      : Drizzt
License     : GXU
"""
...
class LogParser:
    ...
    def parse(self):
        ...
        for idx, line in self.df_log.iterrows():
            count += 1
            logmessage = self.preprocess(line["Content"])
            if not logDict.get(logmessage):
                logDict[logmessage] = [idx, ]
            else:
                logDict[logmessage].append(idx)
            if count % self.batch_size == 0 or count >= len(self.df_log):
                logmessageL = list(logDict.keys())
                random.seed(self.index)
                random.shuffle(logmessageL)
                for item in logmessageL:
                    message = item.split()
                    ...
                    #Add the new log message to the existing cluster
                        ...
                        newTemplate = self.getTemplate(message, matchCluster.logTemplate)
                        matchCluster.logIDL += logDict[item]
    ...
    def preprocess(self, line):
        ...
        if index_length > length:
            line += " _"*(index_length-length+1)
        word_list = line.strip().split()
        foobar = word_list[self.index:] + word_list[:self.index]
        return " ".join(foobar)
